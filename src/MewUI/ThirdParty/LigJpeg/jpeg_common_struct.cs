...
/// <summary>Base class for JPEG decompressor.</summary>
/// <remarks>
...
public abstract class jpeg_common_struct
{
    ...
    internal jpeg_error_mgr m_err = null!;
    ...
    /// <summary>
    /// Creates 2-D sample array (legacy jagged array version).
    /// </summary>
    ...
    public static byte[][] AllocJpegSamples(int samplesPerRow, int numberOfRows)
    {
        ...
        {
            result[i] = GC.AllocateUninitializedArray<byte>(samplesPerRow);
        }
    ...
    /// <summary>
    /// Creates a contiguous 2-D sample buffer with better cache locality.
    /// </summary>
    /// <param name="samplesPerRow">The number of samples per row.</param>
    /// <param name="numberOfRows">The number of rows.</param>
    /// <returns>A contiguous Buffer2D.</returns>
    public static Buffer2D<byte> AllocSampleBuffer(int samplesPerRow, int numberOfRows)
    {
        return new Buffer2D<byte>(samplesPerRow, numberOfRows);
    }
    ...
    public void jpeg_abort()
    {
        ...
        // No extra reset needed for this fork.
    }
