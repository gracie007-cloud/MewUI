...
internal sealed class my_merged_upsampler
{
    ...
    */
    private byte[] m_spare_row = Array.Empty<byte>();
    private bool m_spare_full;        /* T if spare buffer is occupied */

    // Avoid per-call jagged array allocations in merged_2v_upsample.
    private readonly byte[][] m_oneRow = new byte[1][];
    private readonly byte[][] m_twoRows = new byte[2][];
    ...
    public bool NeedContextRows() => m_need_context_rows;
    ...
    public void upsample(ComponentBuffer[] input_buf, ref int in_row_group_ctr, int in_row_groups_avail, byte[][] output_buf, ref int out_row_ctr, int out_rows_avail)
    {
    ...
    private void merged_2v_upsample(ComponentBuffer[] input_buf, ref int in_row_group_ctr, byte[][] output_buf, ref int out_row_ctr, int out_rows_avail)
    {
        ...
        if (m_spare_full)
        {
            ...
            m_oneRow[0] = m_spare_row;
            JpegUtils.jcopy_sample_rows(m_oneRow, 0, output_buf, out_row_ctr, 1, m_out_row_width);
            num_rows = 1;
            m_spare_full = false;
        }
        else
        {
            ...
            m_twoRows[0] = output_buf[out_row_ctr];
            if (num_rows > 1)
            {
                m_twoRows[1] = output_buf[out_row_ctr + 1];
            }
            else
            {
                m_twoRows[1] = m_spare_row;
                m_spare_full = true;
            }

            /* Now do the upsampling. */
            h2v2_merged_upsample(input_buf, in_row_group_ctr, m_twoRows);
        }
