...
/// </summary>
internal sealed class jpeg_marker_reader
{
        ...
        for (; ; )
        {
            ...
            switch ((JPEG_MARKER)m_cinfo.m_unread_marker)
            {
                ...
                case JPEG_MARKER.APP15:
                    if (!process_appn(m_cinfo.m_unread_marker - (int)JPEG_MARKER.APP0))
                    {
                        return ReadResult.JPEG_SUSPENDED;
                    }
    ...
    */
    ...
    private bool process_appn(int appnIndex)
    {
        // Only APP0 (JFIF) and APP14 (Adobe) are interesting for this fork.
        if (appnIndex == 0 || appnIndex == 14)
        {
            return get_interesting_appn(m_cinfo);
        }

        return skip_variable(m_cinfo);
    }
    ...
    private static bool get_interesting_appn(jpeg_decompress_struct cinfo)
    {
        ...
        Span<byte> b = stackalloc byte[APPN_DATA_LEN];
        b.Clear();
    ...
    /// </summary>
    private static void examine_app0(jpeg_decompress_struct cinfo, ReadOnlySpan<byte> data, int datalen, int remaining)
    {
    ...
    /// datalen is # of bytes at data[], remaining is length of rest of marker data.
    /// </summary>
    private static void examine_app14(jpeg_decompress_struct cinfo, ReadOnlySpan<byte> data, int datalen, int remaining)
    {
    ...
    private bool get_sof(bool is_baseline, bool is_prog, bool is_arith)
    {
        ...
        if (is_arith)
        {
            throw new NotSupportedException("Arithmetic-coded JPEG is not supported in this decoder-only fork.");
        }
        ...
        for (int ci = 0; ci < m_cinfo.m_num_components; ci++)
        {
            ...
            jpeg_component_info compptr = m_cinfo.Comp_info![0];
    ...
    private bool get_dht()
    {
        ...
        Span<byte> bits = stackalloc byte[17];
        Span<byte> huffval = stackalloc byte[256];
        while (length > 16)
        {
            int index;
            ...
            bits.CopyTo(htblptr.Bits);
            huffval.CopyTo(htblptr.Huffval);
        }
