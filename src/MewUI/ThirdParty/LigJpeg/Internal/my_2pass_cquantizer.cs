...
class my_2pass_cquantizer : jpeg_color_quantizer
{
    ...
    */
    ...
    private byte[][] m_sv_colormap = null!;  /* colormap allocated at init time */
    ...
    private void pass2_fs_dither(byte[][] input_buf, int in_row, byte[][] output_buf, int out_row, int num_rows)
    {
        ...
        var colormap = m_cinfo.m_colormap!;
    ...
    private void compute_color(box[] boxlist, int boxIndex, int icolor)
    {
        ...
        var colormap = m_cinfo.m_colormap!;
        colormap[0][icolor] = (byte)((c0total + (total >> 1)) / total);
        colormap[1][icolor] = (byte)((c1total + (total >> 1)) / total);
        colormap[2][icolor] = (byte)((c2total + (total >> 1)) / total);
    }
    ...
    private void find_best_colors(int minc0, int minc1, int minc2, int numcolors, byte[] colorlist, byte[] bestcolor)
    {
        ...
        for (int i = 0; i < numcolors; i++)
        {
            ...
            int inc0 = (minc0 - colormap[0][icolor]) * R_SCALE;
            int dist0 = inc0 * inc0;

            int inc1 = (minc1 - colormap[1][icolor]) * G_SCALE;
            dist0 += inc1 * inc1;

            int inc2 = (minc2 - colormap[2][icolor]) * B_SCALE;
            dist0 += inc2 * inc2;
