...
struct jpeg_input_controller
{
    ...
    /// </summary>
    public jpeg_input_controller(jpeg_decompress_struct cinfo) : this()
    {
        m_cinfo = cinfo;
    ...
    */
    ...
    private void latch_quant_tables()
    {
        for (int ci = 0; ci < m_cinfo.m_comps_in_scan; ci++)
        {
            ...
            var srcQTable = m_cinfo.m_quant_tbl_ptrs[qtblno]!;

            /* OK, save away the quantization table */
            JQUANT_TBL qtbl = new JQUANT_TBL();
            Buffer.BlockCopy(srcQTable.quantval, 0,
                qtbl.quantval, 0, qtbl.quantval.Length * sizeof(short));
            qtbl.Sent_table = srcQTable.Sent_table;
