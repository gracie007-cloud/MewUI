...
/// </summary>
internal sealed class huff_entropy_decoder
{
    private enum DecodeMcuMethod : byte
    {
        None = 0,
        Full = 1,
        Sub = 2,
    ...
    */
    ...
    private unsafe struct savable_state
    {
        public uint EOBRUN; /* remaining EOBs in EOBRUN */

        private fixed int m_last_dc_val[JpegConstants.MAX_COMPS_IN_SCAN]; /* last DC coef for each component */

        public Span<int> LastDcVal
        {
            get
            {
                fixed (int* p = m_last_dc_val)
                {
                    return new Span<int>(p, JpegConstants.MAX_COMPS_IN_SCAN);
                }
            }
        }
    }
    ...
    */
    ...
    private DecodeMcuMethod m_decodeMcuMethod;
    ...
    {
        m_cinfo = cinfo;
        m_decodeMcuMethod = DecodeMcuMethod.None;
        ...
        else
        {
            ...
            for (int i = 0; i < JpegConstants.NUM_HUFF_TBLS; i++)
            {
                m_dc_derived_tbls[i] = default;
                m_ac_derived_tbls[i] = default;
            }
        }
    }
    ...
    public void start_pass()
    {
        if (m_cinfo.m_progressive_mode)
        {
            ...
            var compInfo = m_cinfo.Comp_info!;
            ...
            {
                int cindex = compInfo[m_cinfo.m_cur_comp_info[ci]].Component_index;
                var coefBits = m_cinfo.m_coef_bits!;

                if (m_cinfo.m_Ss != 0 && coefBits[cindex][0] < 0)
                {
                ...
                {
                    int expected = coefBits[cindex][coefi];
                    if (expected < 0)
                    {
                        expected = 0;
                    }
                    ...
                    coefBits[cindex][coefi] = m_cinfo.m_Al;
                }
            }
            ...
            if (m_cinfo.m_Ah == 0)
            {
                if (m_cinfo.m_Ss == 0)
                {
                    m_decodeMcuMethod = DecodeMcuMethod.DcFirst;
                }
                else
                {
                    m_decodeMcuMethod = DecodeMcuMethod.AcFirst;
                }
            }
            else
            {
                if (m_cinfo.m_Ss == 0)
                {
                    m_decodeMcuMethod = DecodeMcuMethod.DcRefine;
                }
                else
                {
                    m_decodeMcuMethod = DecodeMcuMethod.AcRefine;
                }
            }
        ...
        else
        {
            ...
            {
                m_decodeMcuMethod = DecodeMcuMethod.Sub;
            }
            else
            {
                m_decodeMcuMethod = DecodeMcuMethod.Full;
            }
    ...
    public bool decode_mcu(JBLOCK[] MCU_data)
        => m_decodeMcuMethod switch
        {
            DecodeMcuMethod.Full => decode_mcu_full(MCU_data),
            DecodeMcuMethod.Sub => decode_mcu_sub(MCU_data),
            DecodeMcuMethod.DcFirst => decode_mcu_DC_first(MCU_data),
            DecodeMcuMethod.AcFirst => decode_mcu_AC_first(MCU_data),
            DecodeMcuMethod.DcRefine => decode_mcu_DC_refine(MCU_data),
            DecodeMcuMethod.AcRefine => decode_mcu_AC_refine(MCU_data),
            _ => false,
        };

    public void finish_pass() => finish_pass_huff();
    ...
    private bool decode_mcu_full(JBLOCK[] MCU_data)
    {
        ...
        if (!m_insufficient_data)
        {
            ...
            for (int blkn = 0; blkn < m_cinfo.m_blocks_in_MCU; blkn++)
            {
                ...
                ref d_derived_tbl dc_tbl = ref m_dc_cur_tbls[blkn];
                int s = HUFF_DECODE(ref dc_tbl);

                ref d_derived_tbl ac_tbl = ref m_ac_cur_tbls[blkn];
                int k = 1;
                ...
                if (thisBlockCoeffLimit != 0)
                {
                    ...
                    var mcuDataBlock = MCU_data[blkn].Coefficients;
    ...
    private bool decode_mcu_DC_first(JBLOCK[] MCU_data)
    {
        ...
        if (!m_insufficient_data)
        {
            ...
            for (int blkn = 0; blkn < m_cinfo.m_blocks_in_MCU; blkn++)
            {
                ...
                /* Section F.2.2.1: decode the DC coefficient difference */
                ref d_derived_tbl dc_tbl = ref derived_tbls[m_cinfo.Comp_info![m_cinfo.m_cur_comp_info[ci]].Dc_tbl_no];
                int s = HUFF_DECODE(ref dc_tbl);
    ...
    private bool decode_mcu_AC_first(JBLOCK[] MCU_data)
    {
        ...
        if (!m_insufficient_data)
        {
            ...
            else
            {
                ...
                var mcuDataBlock = MCU_data[0].Coefficients;
                for (int k = m_cinfo.m_Ss; k <= m_cinfo.m_Se; k++)
                {
                    int s = HUFF_DECODE(ref ac_derived_tbl);
    ...
    */
    private unsafe int HUFF_DECODE(ref d_derived_tbl htbl)
    {
        int nb = 0;
        bool doSlow = false;
        ...
        return jpeg_huff_decode(ref htbl, nb);
    }
    ...
    /// </summary>
    private unsafe void jpeg_make_d_derived_tbl(bool isDC, int tblno, ref d_derived_tbl dtbl)
    {
        ...
        */
        ...
        JHUFF_TBL? table = isDC ? m_cinfo.m_dc_huff_tbl_ptrs[tblno] : m_cinfo.m_ac_huff_tbl_ptrs[tblno];
        if (table == null)
        {
            m_cinfo.ERREXIT(J_MESSAGE_CODE.JERR_NO_HUFF_TABLE, tblno);
            return;
        ...
        */
        ...
        for (int i = 0; i < (1 << JpegConstants.HUFF_LOOKAHEAD); i++)
        {
            dtbl.look_nbits[i] = 0;
            dtbl.look_sym[i] = 0;
        }
        ...
        for (int l = 1; l <= JpegConstants.HUFF_LOOKAHEAD; l++)
        {
            for (int i = 1; i <= htbl.Bits[l]; i++, p++)
            {
                ...
                {
                    dtbl.look_nbits[lookbits] = (byte)l;
    ...
    private unsafe int jpeg_huff_decode(ref d_derived_tbl htbl, int min_bits)
    {
