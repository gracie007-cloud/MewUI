...
struct jpeg_d_main_controller
{
    ...
    public jpeg_d_main_controller(jpeg_decompress_struct cinfo) : this()
    {
        m_cinfo = cinfo;
        ...
        */
        ...
        ref var upsample = ref cinfo.m_upsample;
        if (upsample.NeedContextRows())
        {
    ...
    public void start_pass(J_BUF_MODE pass_mode)
    {
        switch (pass_mode)
        {
            case J_BUF_MODE.JBUF_PASS_THRU:
                ref var upsample = ref m_cinfo.m_upsample;
                if (upsample.NeedContextRows())
                {
    ...
    /// <summary>
    /// Process data directly into a contiguous Buffer2D.
    /// </summary>
    public void process_data(Buffer2D<byte> output_buf, int startRow, ref int out_row_ctr, int out_rows_avail)
    {
        // Create temporary row pointers into the Buffer2D
        int rowsNeeded = out_rows_avail;
        int rowWidth = output_buf.Width;

        // Rent arrays from pool to avoid allocation
        byte[][] tempRows = ArrayPool<byte[]>.Shared.Rent(rowsNeeded);
        byte[]?[] rentedBuffers = ArrayPool<byte[]?>.Shared.Rent(rowsNeeded);

        try
        {
            for (int i = 0; i < rowsNeeded; i++)
            {
                rentedBuffers[i] = ArrayPool<byte>.Shared.Rent(rowWidth);
                tempRows[i] = rentedBuffers[i]!;
            }

            // Process using legacy path
            process_data(tempRows, ref out_row_ctr, out_rows_avail);

            // Copy results to Buffer2D
            for (int i = 0; i < out_row_ctr; i++)
            {
                tempRows[i].AsSpan(0, rowWidth).CopyTo(output_buf.GetRow(startRow + i));
            }
        }
        finally
        {
            ...
            for (int i = 0; i < rowsNeeded; i++)
            {
                if (rentedBuffers[i] != null)
                {
                    ArrayPool<byte>.Shared.Return(rentedBuffers[i]!);
                }
            }
            ArrayPool<byte[]?>.Shared.Return(rentedBuffers);
            ArrayPool<byte[]>.Shared.Return(tempRows);
        }
    }
    ...
    {
        int dummy = 0;
        m_cinfo.m_post.post_process_data(Array.Empty<ComponentBuffer>(), ref dummy, 0, output_buf, ref out_row_ctr, out_rows_avail);
    }
