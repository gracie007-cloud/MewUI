    ...
    public void prepare_for_output_pass()
    {
        ...
        else
        {
            if (m_cinfo.m_quantize_colors && m_cinfo.m_colormap == null)
            {
                ...
                {
                    m_cinfo.m_cquantize = m_quantizer_2pass ?? throw new InvalidOperationException("2-pass quantizer not initialized");
                    m_is_dummy_pass = true;
                }
                else if (m_cinfo.m_enable_1pass_quant)
                {
                    m_cinfo.m_cquantize = m_quantizer_1pass ?? throw new InvalidOperationException("1-pass quantizer not initialized");
                }
                else
                {
    ...
    private void master_selection()
    {
        ...
        if (!m_cinfo.m_raw_data_out)
        {
            if (m_using_merged_upsample)
            {
                ...
                m_cinfo.m_upsample = jpeg_upsampler_union.CreateMyMergedUpsampler(m_cinfo);
            }
            else
            {
                m_cinfo.m_cconvert = new jpeg_color_deconverter(m_cinfo);
                m_cinfo.m_upsample = jpeg_upsampler_union.CreateMyUpsampler(m_cinfo);
            }
        ...
        m_cinfo.m_entropy = jpeg_entropy_decoder_union.CreateHuffman(m_cinfo);
