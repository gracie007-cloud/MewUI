...
class my_1pass_cquantizer : jpeg_color_quantizer
{
    ...
    */
    ...
    private byte[][] m_sv_colormap = null!;	/* The color map as a 2-D pixel array */
    private int m_sv_actual;		/* number of entries in use */

    private byte[][] m_colorindex = null!;	/* Precomputed mapping for speed */
    private int[] m_colorindexOffset = null!;
    ...
    */
    ...
    {
        var outBuf = output_buf!;
        switch (m_quantizer)
        {
            case QuantizerType.color_quantizer3:
                quantize3(input_buf, in_row, outBuf, out_row, num_rows);
                break;
            case QuantizerType.color_quantizer:
                quantize(input_buf, in_row, outBuf, out_row, num_rows);
                break;
            case QuantizerType.quantize3_ord_dither_quantizer:
                quantize3_ord_dither(input_buf, in_row, outBuf, out_row, num_rows);
                break;
            case QuantizerType.quantize_ord_dither_quantizer:
                quantize_ord_dither(input_buf, in_row, outBuf, out_row, num_rows);
                break;
            case QuantizerType.quantize_fs_dither_quantizer:
                quantize_fs_dither(input_buf, in_row, outBuf, out_row, num_rows);
                break;
            default:
